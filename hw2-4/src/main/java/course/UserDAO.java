...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        Document toInsertDoc = new Document("_id", username).append("password", passwordHash);
        
        if (email != null && !email.equals("")) {
            // XXX WORK HERE
            // if there is an email address specified, add it to the document too.
        	toInsertDoc = toInsertDoc.append("email", email);
        }

        try {
            // XXX WORK HERE
            // insert the document into the user collection here
        	usersCollection.insertOne(toInsertDoc);
            return true;
